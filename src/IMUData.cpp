#include "imudata.h"

//根据所用IMU的标定结果修改这些参数的值

//measurement的协方差矩阵由观测中连续的噪声产生
//从连续数据的噪声密度中,

    /**
     * For EuRoc dataset, according to V1_01_easy/imu0/sensor.yaml
     * The params:
     * sigma_g: 1.6968e-4       rad / s / sqrt(Hz)
     * sigma_gw: 1.9393e-5      rad / s^2 / sqrt(Hz)
     * sigma_a: 2.0e-3          m / s^2 / sqrt(Hz)
     * sigma_aw: 3.0e-3         m / s^3 / sqrt(Hz)
     */


double IMUData::_gyrBiasRw2 =  2.0e-5 * 2.0e-5 * 10;  //2e-12*1e3
double IMUData::_accBiasRw2 =  5.0e-3 * 5.0e-3 * 10;  //4.5e-8*1e2



Matrix3d IMUData::_gyrMeasCOV = Eigen::Matrix3d::Identity()*1.7e-4*1.7e-4/0.005*100;		//sigma_g * sigma_a/dt  -6e-6*10
Matrix3d IMUData::_accMeasCOV = Eigen::Matrix3d::Identity()*2.0e-4*2.0e-4/0.005*100;		//sigma_a * sigma_a/dt  4.5e-8*1e2

//协方差的bias random walk

Matrix3d IMUData::_gyrBiasRWCOV = Matrix3d::Identity()*_gyrBiasRw2;
Matrix3d IMUData::_accBiasRWCOV = Matrix3d::Identity()*_accBiasRw2;